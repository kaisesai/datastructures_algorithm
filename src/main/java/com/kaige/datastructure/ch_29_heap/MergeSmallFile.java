...
public class MergeSmallFile {
   ...
   * 堆的默认容量
   */
  public static final int HEAP_INITIAL_CAPACITY = 3;
  
  /**
   * 使用优先级队列，采用默认的自然排序——小顶堆
   *
   * @ps 注意这里初始化了堆容量大小，但是当有数据添加到堆中的时候，堆会自动扩容。
   * <p>
   * 如果要实现成只允许固定大小的堆容量，那就需要记录每个文件，读取流读取的数据与流的映射关系，比如建立一个 model，里边有数据和流的字段，
   * <p>
   * 然后将 model 放入堆，这样，当从优先级队列中弹出堆顶数据时，也就自然的获取到了其对应的流，这样再通过这个流读取数据，把数据放入
   * <p>
   * 堆中。
   */
  ...
  public static void main(String[] args) {
    ...
    String newFile
      = "/Users/kaige/IdeaProjects/datastructures_algorithm/src/main/java/com/kaige/datastructure/ch_29_heap/resources/newFile.txt";
